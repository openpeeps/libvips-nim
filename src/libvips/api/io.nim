...
proc savePNG*(img: Image, compression: range[0..9] = pngDefaultCompression): seq[uint8] =
  var buf: pointer
  var len: csize_t
  let rc = vips_pngsave_buffer_safe(img.v, addr buf, addr len)
...
proc saveWebP*(img: Image, quality: range[0..100] = webpDefaultQuality): seq[uint8] =
  var buf: pointer
  var len: csize_t
  let rc = vips_webpsave_buffer_safe(img.v, addr buf, addr len)
